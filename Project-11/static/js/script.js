...
async function fetchPlaylists() {
    ...
    for (let i = 0; i < playlistLinksArray.length; i++) {
        ...
        if (playlistLink.href.includes('/songs/') && (!(playlistLink.href.includes('.htaccess')))) {
            ...
            let folderName = playlistLink.innerHTML.slice(0, playlistLink.innerHTML.length - 1);
            let imgSource = `/songs/${folderName}/${folderName}-img.jpeg`;
...
async function fetchSongs(playlistName) {
    ...
    for (let index = 0; index < songLinks.length; index++) {
        ...
        if (
            (songLinks[index].href.includes(`/songs/`)) 
            && 
            (
                !(songLinks[index].href.includes(`cover`))
                 && 
                !(songLinks[index].href.includes(`.json`))
            ) &&( (songLinks[index].href != `/songs/`))) 
            {
...
async function playSong(song) {
    ...
    // currentSong.addEventListener('ended', preNxt('nxt'));
...
function seekBarTime() {
    ...
    if (currentSong != null) {
        ...
        // console.trace();
        runningSongTime = setInterval(async() => {
