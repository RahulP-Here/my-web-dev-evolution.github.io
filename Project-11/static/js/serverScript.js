...
async function fetchSongs(playlistName) {
    ...
    for (let index = 0; index < songLinks.length; index++) {
        if ((songLinks[index].href.includes(`/songs/`)) && (!(songLinks[index].href.includes(`cover`)) && !(songLinks[index].href.includes(`.json`)) && (songLinks[index].href != `https://spotifyclonerp.freewebhostmost.com/songs/`))) {
