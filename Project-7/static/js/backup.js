        ...
        if (playlistLink.innerHTML !== "../") {
            ...
            albumElement.innerHTML = `
                <div class="image">
                    ...
                    <img src="./songs/${albumName}/${albumName}-img.jpeg">
                </div>
                <h3>${albumName}</h3>
    ...
    for (let index = 0; index < songLinks.length; index++) {
        if ((index > 0) && !(((songLinks[index].innerHTML).includes('-img.')))) {
            ...
            songElement.innerHTML = `
                <img src="./static/img/album logo.png" alt="song">
...
async function main() {
    ...
    wave.addEventListener('click', async () => {
        if (currentSong) {
            // if song is in playing state SongWave Video Start
            if(!wave.paused){
                await currentSong.pause();
                await wave.pause();
            }
            // if song is in pause state SongWave Video Pause
            else{
                await currentSong.play();
                await wave.play();
            }
